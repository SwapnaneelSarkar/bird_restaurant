...
class AddProductFromCatalogScreen extends StatefulWidget {
  const AddProductFromCatalogScreen({Key? key}) : super(key: key);
...
class _AddProductFromCatalogScreenState extends State<AddProductFromCatalogScreen> {
  ...
  Future<void> _loadRestaurantInfo() async {
    try {
      ...
      if (mounted) {
        ...
        debugPrint('🔄 AddProductFromCatalogPage: Loaded restaurant info - Name: ${info['name']}, Slogan: ${info['slogan']}, Image: ${info['imageUrl']}');
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: BlocProvider(
        create: (context) => AddProductFromCatalogBloc()..add(const AddProductFromCatalogInitEvent()),
        child: BlocConsumer<AddProductFromCatalogBloc, AddProductFromCatalogState>(
          ...
          builder: (context, state) {
            debugPrint('AddProductFromCatalogScreen: BlocConsumer builder state = \u001b[36m'+state.runtimeType.toString()+'\u001b[0m');
            if (state is AddProductFromCatalogFormState) {
              return Scaffold(
                key: _scaffoldKey,
                backgroundColor: ColorManager.background,
                drawer: SidebarDrawer(
                  activePage: 'addProductFromCatalog',
                  restaurantName: _restaurantInfo?['name'] ?? 'Restaurant',
                  restaurantSlogan: _restaurantInfo?['slogan'] ?? 'Add product from catalog',
  ...
  Widget _buildForm(BuildContext context, AddProductFromCatalogFormState state) {
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Select a product from the catalog and set your price and quantity',
                    style: TextStyle(
          ...
          if (state.selectedProduct != null) ...[
            ...
            CustomTextField(
              ...
              onChanged: (value) {
                ...
                context.read<AddProductFromCatalogBloc>().add(QuantityChangedEvent(quantity));
                context.read<AddProductFromCatalogBloc>().add(ValidateQuantityEvent(quantity));
              },
            ),
            const SizedBox(height: 16),
            ...
            _buildToggleOption(
              ...
              'Available',
              state.available,
              (value) => context.read<AddProductFromCatalogBloc>().add(ToggleAvailableEvent(value)),
            ),
              ...
              children: [
                  ...
                  child: ElevatedButton(
                    onPressed: state.isSubmitting
                        ...
                        : () {
                             ...
                             context.read<AddProductFromCatalogBloc>().add(ValidateAllFieldsEvent());
                             if (_isFormValid(state)) {
                               context.read<AddProductFromCatalogBloc>().add(SubmitProductEvent());
                             } else {
                ...
                Expanded(
                  child: OutlinedButton(
                                         onPressed: () {
                       context.read<AddProductFromCatalogBloc>().add(ResetFormEvent());
                       _resetControllers();
                     },
  ...
  bool _isFormValid(AddProductFromCatalogFormState state) {
  ...
  Widget _buildCategoryDropdown(BuildContext context, AddProductFromCatalogFormState state) {
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<CategoryModel>(
          ...
          onChanged: (CategoryModel? newValue) {
            if (newValue != null) {
              context.read<AddProductFromCatalogBloc>().add(CategorySelectedEvent(newValue));
            }
          },
        ),
      ),
  ...
  Widget _buildSubcategoryDropdown(BuildContext context, AddProductFromCatalogFormState state) {
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<SubcategoryModel>(
          ...
          onChanged: (SubcategoryModel? newValue) {
            if (newValue != null) {
              context.read<AddProductFromCatalogBloc>().add(SubcategorySelectedEvent(newValue));
            }
          },
        ),
      ),
  ...
  Widget _buildProductDropdown(BuildContext context, AddProductFromCatalogFormState state) {
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<ProductSelectionModel>(
          ...
          onChanged: (ProductSelectionModel? newValue) {
            if (newValue != null) {
              context.read<AddProductFromCatalogBloc>().add(ProductSelectedEvent(newValue));
            }
          },
        ),
      ),
  ...
  Widget _buildProductDetails(BuildContext context, AddProductFromCatalogFormState state) {
  ...
  Widget _buildPriceField(BuildContext context, AddProductFromCatalogFormState state) {
    return Container(
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(8),
          ...
          Expanded(
            child: TextFormField(
              ...
              onChanged: (value) {
                ...
                context.read<AddProductFromCatalogBloc>().add(PriceChangedEvent(price));
                context.read<AddProductFromCatalogBloc>().add(ValidatePriceEvent(price));
              },
            ),
          ),
        ],
